...
namespace Pathfinding {
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		...
		public virtual void OnTargetReached () {
            GetComponent<IActionController>().Arrived();
		}
		...
		protected virtual void Update () {
            ...
            if((previousPosition1 - previousPosition2).magnitude > 0){
                anim.SetBool("walking", true);
                if (Mathf.Abs((previousPosition1 - previousPosition2).x) > 0.001){
                    sprite.flipX = !((previousPosition1 - previousPosition2).x < 0);
                } else {
                    sprite.flipX = !(transform.position.x > destination.x);
                }
            } else {
